...
- **Auto-stop sheds only the speed it needs** — it solves for the fastest you can still move and make the shot, so a close one barely slows you. Past the range your hit chance is reachable at all it plants you, rather than quietly handing back full speed in exactly the situations you switched it on for. Bots keep closing instead of rooting on a shot that rooting can't win.
- **Squad roles, not one shared plan** — every round each bot draws a role (push · hold · flank) and a *lateral* lane, and leaves spawn on its own staggered clock. Lanes run across the map while depth runs along it, so a team fans out sideways and still advances; holders plant on an angle in their lane and watch it until the round goes quiet. At matched survivor counts the share of a team bunched within 250 units falls from 41%/32%/53% to 32%/24%/23% at 8s/12s/16s.
...
- **Anti-aim that does something.** Six yaw modes (back · sideways · spin · jitter · sway · rand) with a jitter/sway range that drives the body, a desync angle that is now real geometry (0° is genuinely no fake; 58° swings it about a body's width off you), **freestanding** — which looks at the map and puts the fake where the nearest enemy can see it, leaving the real you behind the corner — and **fake duck**, which holds you *really* ducked while the model keeps standing — your hitboxes sit a stance-height below whatever anyone is aiming at.  It needs neither desync nor a desync angle: the sideways fake and the vertical one are separate tricks.  It is a **bind** (`X` by default, hold or toggle), because while it is engaged you are genuinely crouched and genuinely slow — a switch you leave on is a switch that quietly halves your movement speed.  A faster yaw, a wider desync, a fake pitch and a fake duck each cost a resolver something.
...
- **A lost read costs the head, not the body.** A desync is the lower body rotating about the spine, not the whole player teleporting sideways, so how far a hitbox moves depends on how far it sits from that axis: the head is out at the end of the lever and swings clear, while the chest and stomach barely move and their real and fake boxes still overlap.  That overlap *is* the safe point, and it is why HvH players body-aim a desyncer — a blind body shot lands ~96% of the time where a blind head shot lands ~0%.
- **Per-weapon aimbot config** — hit chance, min damage and hitbox priority have a master value and a per-weapon override.  Pick a gun in the Rage tab and switch on just the settings you want to pin; everything you leave alone keeps following the master, so raising the master still raises every weapon you have not touched.
...
**Crouch** is bound to `Ctrl` **or** `C` — use `C` if your browser closes the tab on `Ctrl+W`.  Ducking only slows you with your feet on the ground: a crouch-jump keeps every unit of the speed you left the ground with, as in CS.  **Fake duck** is its own bind (`X` by default), configurable in the Anti-Aim tab.
