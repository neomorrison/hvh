...
export function makeBodyGLB(team, isHuman) {
  ...
  const bone = n => g.getObjectByName(n) || null;
  ...
  const holder = new THREE.Group(); if (handR) { handR.add(holder); holder.position.set(0, -2, 6); holder.rotation.set(0, Math.PI, 0); } else g.add(holder);
  ...
  return { g, upper: spine || g, legs: hips || g, head: head || clothHolder, chest: clothHolder, belly: clothHolder, holder, weapon: null,
           glb: true, mixer, actions, cur: 'idle', bones: { hips, spine, head } };
}
/* Drive a GLB body: blend to the right clip from movement state, apply anti-aim twist on the spine bone. */
